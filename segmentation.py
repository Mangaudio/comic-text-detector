...
# returns cropped text images
def segment_image(model: TextDetector, image_path: str) -> list[TextBubble]:
    ...
    for blk in blk_list:
        ...
        bubbles.append(
            TextBubble(
                xyxy,
                blk.text,
                PIL.Image.fromarray(img[xyxy[1] : xyxy[3], xyxy[0] : xyxy[2]]),
            )
        )
        # save image for test
        bubbles[i].image.save(f"bubble_{i}.png")
    return bubbles


if __name__ == "__main__":
    model_path = "data/comictextdetector.pt"
    image_path = r"data/153_.jpg"
    cuda = torch.cuda.is_available()
    device = "cuda" if cuda else "cpu"
    model = TextDetector(
        model_path=model_path, input_size=1024, device=device, act="leaky"
    )
    bubbles = segment_image(model, image_path)
    print(bubbles)
